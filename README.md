...
Important note: Please create a new ArcGIS Pro Project (and make sure to check the "Create a folder for this local project" checkbox), and then inside it create a Map (it's one of the options). Afterwards, change the values of the 2 following variables in the code (the code is in a python file called 'The_actual_code.py'; it's in this repository):
...
Afterwards, please SAVE AND CLOSE the project and then run the code attached (that is, run the python file 'The_actual_code.py', which is in this repository).
